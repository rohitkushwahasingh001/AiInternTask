...
class DocumentProcessor:
    ...
    async def process_document(self, file_path: str, filename: str) -> Tuple[str, List[Dict]]:
        ...
        document_content = [] # To store text content for the entire document, page by page or block by block
    ...
    async def process_documents(self, files: List[UploadFile]) -> List[Dict]:
        ...
        for file in files:
            try:
                ...
                # Here, instead of adding to DB directly, return chunks to routes.py
                processed_docs_info.append({
                    "document_id": document_id,
                    "filename": file.filename,
                    "chunks": chunks # Return chunks to be added by VectorDBService in routes.py
                })
